  ...
  <el-container>
    ...
    <el-aside width="200px">
      <el-menu :default-active="this.$route.path" router>
        <el-menu-item key="Home" index="/main">
          <i class="el-icon-s-home"></i>
          <span slot="title">Home</span>
        </el-menu-item>
        <el-menu-item key="Dataset" index="/">
          <i class="el-icon-s-data"></i>
          <span slot="title">Dataset</span>
        </el-menu-item>
        <el-menu-item key="Generation" index="/">
          <i class="el-icon-s-opportunity"></i>
          <span slot="title">Generation</span>
        </el-menu-item>
        <el-menu-item key="Leaderboard" index="/">
    ...
    <el-container>
      ...
      <el-header>
        <el-row :gutter="20">
          ...
          <el-col :span="2">
            <!-- 点击触发函数getSearch -->
            <el-button type="primary" icon="el-icon-search" @click="getSerach">Search</el-button>
          </el-col>
        </el-row>
      </el-header>
      <!-- 主栏部分 -->
...
import { encodeHTML } from "echarts/lib/util/format";
export default {
  data() {
    return {
      text: "",
  ...
  methods: {
    ...
    handleImageClick(index) {
      if (index === 0) { // 如果点击了第一张图片（索引为0）
        this.$router.push({ name: 'test' }); // 使用 Vue Router 跳转到名为 Dataset 的页面
      }
      // 其他情况，可以添加其他图片点击的逻辑
    },
